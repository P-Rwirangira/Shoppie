...
export const editUser = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        let uploadedImage = user.photoUrl // Keep existing photo if no new one uploaded
        
        // Only process file upload if file is provided
        if ((req as any).file) {
            const uploadResult = await uploadImage((req as any).file.buffer)
